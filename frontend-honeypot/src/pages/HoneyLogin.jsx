...
export default function HoneyLogin({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${API}/honey/auth`, {
        username: form.username,
        password: form.password,
      })
      onLogin(form.username, res.data.session_id)
      navigate('/dashboard')
    } catch {
      // Even on error — log and proceed (any credentials work)
      onLogin(form.username, null)
      navigate('/dashboard')
    } finally {
      setLoading(false)
    }
